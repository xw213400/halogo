...
def gtp_caps(caps):
    return ",".join([gtp_vertex(vertex) for vertex in caps])
...
class Engine(object):
    ...
    def cmd_play(self, arguments):
        ...
        if move:
            ...
            if self.vertex_in_range(vertex):
                legal, caps = self._game.make_move(color, vertex)
                if legal:
                    return gtp_vertex(vertex) + " " + gtp_caps(caps)
    ...
    def cmd_genmove(self, arguments):
        ...
        if c:
            ...
            legal, caps = self._game.make_move(c, move)
            if legal:
                return gtp_vertex(move) + " " + gtp_caps(caps)
        else:
